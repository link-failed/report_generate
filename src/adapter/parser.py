...
class DbtJsonLogAdapter(BaseAdapter):
        ...
        for logfile in self.logs:
            with open(logfile) as f:
                for line in f:
                    ...
                    elif "Began running node" in line:
                        self._parse_query_start_line(line=line)
                        # print("query start line")
                    elif json_data["level"] == "debug":
                        self._parse_query_debug_line(line=line)
                        # print("debug line")
                    elif json_data["level"] == "info":
                        self._parse_query_info_line(line=line)
                        # print("info line")
    ...
    """
        query start line:
            ...
            info: index, start_time, invocation_id, thread_name, total
            flag: has "total" column
        query end line:
            ...
            info: end_time, duration, total_query, rows_affect
            flag: contains msg like "Finished running node"
    """
    ...
    def _parse_query_start_line(self, line):
        json_data = json.loads(line)
        if "invocation_id" in json_data:
            metadata = self.metadatas[self.running_id.replace("\"", "")]
    ...
    def _parse_query_debug_line(self, line):
        self.running_id = re.search(self.running_id_rule, line).group(1).replace("\"", "")
        ...
        if query_duration is not None and query_duration != "" and query_duration != "0":
            ...
            if re.search(rule, line) is not None:
                ...
                data = dict(
                    ...
                    end_time=query_end_time,
                    thread_name=thread_name[7:],
                    rows_effect=rows_effect
                )
                self.df.insert(self.running_id.replace("\"", ""), query_name, **data)
                print("df inserted")

    def _parse_query_info_line(self, line):
        json_data = json.loads(line)
        if "index" in json_data["data"]:
            qindex = int(json_data["data"]["index"])
            query_name = re.search(self.query_name_rule, line).group(1).replace("\"", "")
            total_query = re.search(self.total_query_rule, line)
            metadata = self.metadatas.get(self.running_id)
            if total_query is not None and metadata is not None:
                total_query = int(total_query.group(1))
                # print(metadata.get(query_name))
                metadata.get(query_name)['total_query'] = total_query
                metadata.get(query_name)['query_index'] = qindex
                # metadata[query_name] = {'total_query': total_query, 'query_index': qindex}
